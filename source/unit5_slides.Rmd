...
We use ``interaction(age,interface)`` to tell ggplot to get all four subgroups and produce four histogram overlaying on each other. They look very busy and not very informative. 
...
These graphs are kinda hard to read when we put them all in one plot even with only ``geom_density`` and no ``geom_histogram`` . Another neat way is to put them in separate plot or facet. It is very easy in ggplot. 
